...
function CompRow({ item, onOpen }: { item: CompChange; onOpen: (i: CompChange) => void }): JSX.Element {
  return (
    <button
      onClick={() => onOpen(item)}
      className="w-full text-left flex items-start gap-3 rounded-xl border border-[var(--border)] bg-[var(--surface-raised)] active:border-accent/40 transition-colors p-3"
    >
